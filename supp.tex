...
\begin{figure}[t]
  ...
  \end{center}
  \vspace{-0.cm}
    \caption{\textbf{(MNIST$\rightarrow$SVHN)} Different ground costs which composed the ground cost used in our SROT soft weighting method. The first 300 source samples are outliers. The left plot is the ground cost representing the new ground cost terms based on the classifier prediction. The middle plot represents the Euclidean distance between all source and target samples. The right plot is our total ground cost with $\gamma = \operatorname{max_{1\leq i,j \leq n}}(\|\xx_i - \yy_j\|_{2})$.}
    \label{fig_app:all_ground_costs_MS}
  \vspace{-0cm}
\end{figure}

\subsection{Considered problem}
In this section, we look for an optimal transport map which transforms a measure into another following \cite{seguy2018large}. To get such a map, we can rely on the optimal transport plan of the Kantorovich problem $\Pi$, which defines a map between the source and target samples through the computation of the so-called barycentric projection. The barycentric projection transports source (resp. target) samples to the target (resp. source) samples using the OT plan. For each target sample $\zz_j$, the corresponding source element can be computed as follows: $\Pi_{\zz_j} =\sum_{i=1}^n \pi_{i,j} \xx_i$. We could then define a map $T$ which maps a sample to its corresponding barycentric projection. When the ground cost is the squared Euclidean distance, the map $T$ associated to the barycentric projection of $\Pi$ can be the solution of the famous Monge problem \citep{santambrogio2015optimal, COT_Peyre}:

\begin{definition}[Monge problem]
Consider 2 discrete probability measures $\alpha = \sum_{i=1}^N a_i\delta_{\xx_i}, \beta = \sum_{j=1}^N b_j\delta_{\zz_j}$ and a ground cost $c$. The Monge problem seeks a map $T:\{\zz_1, \dots, \zz_N\} \rightarrow  \{\xx_1, \dots, \xx_N\}$ which minimizes the problem:
\begin{equation}
    \underset{T}{\operatorname{min}} \left\{ \sum_{j=1}^N c(T(\zz_j), \zz_j) b_j: T_{\#\beta} = \alpha \right\},
\end{equation}
\end{definition}
where $T_{ \# \beta}$ is the push-forward measure of $\beta$ by $T$. A map solving the above problem is called a Monge map. The Monge map is intractable in the continuous case. However, we can approximate the Monge map $T$ using a deep neural network $T_\theta$ trained to approximate the barycentric projection of $\beta$ samples to $\alpha$ samples. Formally we are looking for the solution of the problem:
\begin{align}
    \theta^\star &= \underset{\theta}{\operatorname{argmin}} \sum_{i,j} c(\xx_i, T_\theta(\zz_j)) \Pi_{i,j}^\star, \\
...
Intuitively, the map $T_\theta$ would transform the sample $\zz_j$ to its barycentric projection on the source domain. In the case of second type outliers, it is likely that some samples $\zz_j$ would be matched to second type outliers. That is why we want to study the relevance of our SROT methods in this context. Thus the purpose of this experiment is to transform the target samples in clean source samples and not in outliers present in the source measures.
...
\begin{figure}[t]
  ...
  \end{center}
  \vspace{-0.cm}
    \caption{\textbf{(MNIST$\rightarrow$MNISTM)} Different ground costs which composed the ground cost used in our SROT soft weighting method. The first 300 source samples are outliers. The left plot is the ground cost representing the new ground cost terms based on the classifier prediction. The middle plot represents the Euclidean distance between all source and target samples. The right plot is our total ground cost with $\gamma = \operatorname{max_{1\leq i,j \leq n}}(\|\xx_i - \yy_j\|_{2})$.}
...
We consider the class of digits "1" from the MNIST dataset as target domain and the MNIST-M \citep{DANN}, or SVHN datasets \citep{svhn} as source domains. MNIST is composed of 60,000 $28\times 28$ images of handwritten digits and MNIST-M has those same 60,000 MNIST images but with color patches \citep{DANN}. The Street View House Numbers (SVHN) dataset \citep{svhn} consists of 73, 257 $32 \times 32$ images with digits and numbers in natural scenes.

The target dataset is composed of 1700 "1" MNIST digits. Regarding the source dataset, we have 1700 clean "1" digit samples from the MNIST-M or SVHN dataset corresponding to $\alpha_c$ while the remaining 300 samples are outliers corresponding to "1" digits from the MNIST dataset corresponding to $\alpha_o$. The noise rate is set to $15\%$.
...
\begin{figure}[t]
  ...
  \end{center}
  \vspace{-0.25cm}
    \caption{\textbf{(MNIST$\rightarrow$SVHN)} Converged Monge map $T_\theta$ on the MNIST to SVHN task for different OT losses. $\beta$ samples are 1700 MNIST samples while $\alpha$ samples are 1700 clean SVHN images and 300 MNIST second type outliers. Our classifier detects MNIST samples present in the source dataset. Our SROT hard and soft weighting methods do not converged to any MNIST samples contrary to other OT flows.}
...
\begin{figure}[t]
  ...
  \end{center}
  \vspace{-0.25cm}
    \caption{\textbf{(MNIST$\rightarrow$MNIST-M)} Converged Monge map $T_\theta$ on the MNIST to MNIST-M task for different OT losses. $\beta$ samples are 1700 MNIST samples while $\alpha$ samples are 1700 clean MNIST-M images and 300 MNIST second type outliers. Our classifier detects MNIST samples present in the source dataset. Our SROT hard and soft weighting methods do not converged to any MNIST samples contrary to other OT flows.}
...
\begin{figure}[t]
  ...
  \end{center}
  \vspace{-0.cm}
    \caption{\textbf{(MNIST$\rightarrow$SVHN)} Wasserstein distance between push-forward measure $T_{\#\beta}$ from different optimal transport costs and clean $\alpha_c$ ($W(\alpha_c, T_{\#\beta})$) and as well as loss value ($h(\alpha, T_{\#\beta})$) on the MNIST$\rightarrow$SVHN task. $\beta$  samples are 1700 MNIST samples while $\alpha$ samples are 1700 clean SVHN images and 300 MNIST samples. Our methods SROT hard and soft weighting are able to match the clean source distribution $\alpha_c$.}
...
\begin{figure}[t]
  ...
  \end{center}
  \vspace{-0.cm}
    \caption{\textbf{(MNIST$\rightarrow$MNIST-M)} Wasserstein distance between push-forward measure $T_{\#\beta}$ from different optimal transport costs and clean $\alpha_c$ ($W(\alpha_c, T_{\#\beta})$) and as well as loss value ($h(\alpha, T_{\#\beta})$) on the MNIST$\rightarrow$MNIST-M task. $\beta$  samples are 1700 MNIST samples while $\alpha$ samples are 1700 clean MNIST-M images and 300 MNIST samples. Our methods SROT hard and soft weighting are able to match the clean source distribution $\alpha_c$.}
